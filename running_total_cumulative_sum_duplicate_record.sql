...
INSERT INTO Employee.dbo.Sales_duplicate (OrderID, OrderDate, Category, Revenue) VALUES
...
SELECT * FROM Employee.dbo.Sales_duplicate;

-- Q1: Find running Total per transaction

-- Here duplicate will appear so, we have to handle that
-- way1
SELECT *, sum(Revenue) OVER (ORDER BY OrderDate)
FROM EMPLOYEE.dbo.Sales_duplicate;

-- handle Duplicate
-- way2
WITH CTE AS (SELECT OrderDate, sum(Revenue) as revenue
FROM EMPLOYEE.dbo.Sales_duplicate
GROUP BY OrderDate
)
